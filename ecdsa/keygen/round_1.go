...
func (round *round1) getPaillierBlumeProof() {
}
...
func (round *round1) Start() *tss.Error {
	...
	for j, msg := range round.temp.kgRound0Messages {
		r0msg := msg.Content().(*KGRound0Message)
	...
	// BROADCAST commitments, paillier pk + proof; round 1 message
	{
		msg, err := NewKGRound1Message(
			round.PartyID(), cmt.C, &preParams.PaillierSK.PublicKey, preParams.NTildei, preParams.H1i, preParams.H2i, dlnProof1, dlnProof2, paramProof)
		if err != nil {
			return round.WrapError(err, Pi)
		}
