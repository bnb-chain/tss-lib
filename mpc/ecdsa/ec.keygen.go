...
func getPubEncode(keyInfo *keygen.LocalPartySaveData) (*ecdsa.PublicKey, *mpc.MPCPublicKey) {
	pub := &ecdsa.PublicKey{
		Curve: tss.EC(),
		X:     keyInfo.ECDSAPub.X(),
		Y:     keyInfo.ECDSAPub.Y(),
	}
...
func KeygenProc(threshold int, totalCount int) ([]int, *mpc.MPCPublicKey) {
	...
	for _, curPartyId := range pIDs {
		...
		curKey, _ := keygen.GeneratePreParams(1 * time.Minute)
		P = keygen.NewLocalParty(params, outCh, endCh, *curKey).(*keygen.LocalParty)
