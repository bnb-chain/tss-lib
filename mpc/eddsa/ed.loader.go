...
func GetPublicKeyFromSaveData(saveData *keygen.LocalPartySaveData) *edwards.PublicKey {
...
func GetMPCPubFromSaveData(saveData *keygen.LocalPartySaveData) *mpc.FSLMPCPublicKey {
	...
	return &mpc.FSLMPCPublicKey{
		Curve:  tss.Edwards().Params().Name,
		X:      hexPubX,
		Y:      hexPubY,
		Encode: hex.EncodeToString(publicKey.Serialize()),
	}
}
